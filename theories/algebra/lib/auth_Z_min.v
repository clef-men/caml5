...
Definition auth_Z_min_ub n : auth_Z_min_UR :=
  ◯O Build_Z_min n.
...
Proof.
  apply _.
Qed.
#[global] Instance auth_Z_min_ub_core_id n :
  CoreId (auth_Z_min_ub n).
Proof.
  apply _.
Qed.
...
Lemma auth_Z_min_ub_op n1 n2 :
  auth_Z_min_ub (n1 `min` n2) = auth_Z_min_ub n1 ⋅ auth_Z_min_ub n2.
Proof.
  rewrite -auth_option_frag_op Z_min_op_eq //.
Qed.
#[global] Instance auth_Z_min_ub_is_op n n1 n2 :
  IsOp (Build_Z_min n) (Build_Z_min n1) (Build_Z_min n2) →
  IsOp' (auth_Z_min_ub n) (auth_Z_min_ub n1) (auth_Z_min_ub n2).
Proof.
  rewrite /IsOp' /IsOp /auth_Z_min_ub => -> //.
Qed.

Lemma auth_Z_min_auth_frag_op dq n :
  auth_Z_min_auth dq n ≡ auth_Z_min_auth dq n ⋅ auth_Z_min_ub n.
Proof.
  rewrite -!assoc -auth_option_frag_op -core_id_dup //.
Qed.

Lemma auth_Z_min_ub_op_le n n' :
  (n ≤ n')%Z →
  auth_Z_min_ub n = auth_Z_min_ub n' ⋅ auth_Z_min_ub n.
Proof.
  intros. rewrite -auth_Z_min_ub_op Z.min_r //.
Qed.
...
Lemma auth_Z_min_both_dfrac_valid dq n m :
  ✓ (auth_Z_min_auth dq n ⋅ auth_Z_min_ub m) ↔
  ✓ dq ∧ (n ≤ m)%Z.
Proof.
