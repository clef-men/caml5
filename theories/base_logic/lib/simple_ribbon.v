...
Section val.
  ...
  Inductive simple_ribbon_state :=
    | SimpleRibbonEmpty
    | SimpleRibbonSet v
    | SimpleRibbonFree.

  Inductive simple_ribbon_step : simple_ribbon_state → simple_ribbon_state → Prop :=
  ...
  Definition simple_ribbon_steps :=
    rtc simple_ribbon_step.

  Ltac invert_simple_ribbon_steps :=
    repeat match goal with
    | H: simple_ribbon_step _ _ |- _ => invert H
    | H: simple_ribbon_steps _ _ |- _ => invert H
    | H: rtc simple_ribbon_step _ _ |- _ => invert H
    end.

  #[local] Instance simple_ribbon_steps_antisymm :
    AntiSymm (=) simple_ribbon_steps.
  Proof.
    intros s1 s2 Hsteps1 Hsteps2. destruct s1, s2; invert_simple_ribbon_steps; done.
  Qed.

  Class SimpleRibbonG Σ := {
    simple_ribbon_G_ribbon_G : RibbonG simple_ribbon_step Σ ;
  }.
  #[local] Existing Instance simple_ribbon_G_ribbon_G.

  Definition simple_ribbon_Σ := #[
    ribbon_Σ simple_ribbon_step
  ].
  #[global] Instance subG_ribbon_Σ Σ :
    subG simple_ribbon_Σ Σ →
    SimpleRibbonG Σ.
  Proof.
    solve_inG.
  Qed.
  ...
  Section simple_ribbon_G.
    Context `{simple_ribbon_G : !SimpleRibbonG Σ}.

    Definition simple_ribbon_auth γ dq rib :=
      ribbon_auth simple_ribbon_step γ dq rib.
    Definition simple_ribbon_mapsto γ i q s :=
      ribbon_mapsto simple_ribbon_step γ i q s.
    Definition simple_ribbon_lb γ i s :=
      ribbon_lb simple_ribbon_step γ i s.
    ...
    Lemma simple_ribbon_auth_lb_valid γ dq rib i t :
      ...
      ⌜∃ s, rib !! i = Some s ∧ simple_ribbon_steps t s⌝.
    Proof.
...
#[global] Arguments simple_ribbon_steps : clear implicits.
#[global] Arguments SimpleRibbonG : clear implicits.
#[global] Arguments simple_ribbon_Σ : clear implicits.
