...
Section auth_nat_max_G.
  ...
  Proof.
    iIntros "Hauth1 Hauth2".
    iDestruct (auth_nat_max_auth_valid_2 with "Hauth1 Hauth2") as %?. naive_solver.
  Qed.
  ...
  Lemma auth_nat_max_auth_ne γ1 n1 γ2 dq2 n2 :
    ...
    ⌜γ1 ≠ γ2⌝.
  Proof.
    intros. iApply auth_nat_max_auth_dfrac_ne; [done.. | intros []%(exclusive_l _)].
  Qed.
  ...
  Proof.
    iIntros "Hauth1 Hauth2".
    iDestruct (auth_nat_max_auth_valid_2 with "Hauth1 Hauth2") as %(? & _). done.
  Qed.
  ...
  Proof.
    iIntros "% Hauth".
    iMod (own_update with "Hauth"); first apply auth_nat_max_auth_update; done.
  Qed.
End auth_nat_max_G.
